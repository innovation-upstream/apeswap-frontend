...
const OverallFigures: React.FC<any> = (props) => {
  ...
  function processChartData() {
    ...
    for (let i = 0; i < Object.keys(chartData).length; i++) {
      const chainId = Object.keys(chartData)[i]

      if (activeChains?.includes(Number(chainId))) {
        if (data.length === 0) {
          for (let j = 0; j < chartData[chainId].data.length; j++) {
            data.push({
              date: chartData[chainId].data[j].date,
              [chainId]: chartData[chainId].data[j].dailyVolumeUSD,
            })
          }
        } else {
          for (let j = 0; j < chartData[chainId].data.length; j++) {
            data[j][chainId] = chartData[chainId].data[j].dailyVolumeUSD
          }
        }
      }
    }
      ...
      if (activeChains?.includes(Number(chainId))) {
        if (data.length === 0) {
          for (let j = 0; j < chartData[chainId].data.length; j++) {
            data.push({
              date: chartData[chainId].data[j].date,
              [chainId]: chartData[chainId].data[j].totalLiquidityUSD,
            })
          }
        } else {
          for (let j = 0; j < chartData[chainId].data.length; j++) {
            data[j][chainId] = chartData[chainId].data[j].totalLiquidityUSD
          }
        }
      }
    }
  ...
  function getLiquidity() {
    ...
    for (let i = 0; i < Object.keys(chartData).length; i++) {
      ...
      if (activeChains?.includes(Number(chainId))) {
        liquidity +=
          Number(chartData[chainId].data[Object.keys(chartData).length].totalLiquidityUSD) -
          Number(chartData[chainId].data[0].totalLiquidityUSD)
      }
    }
  ...
  function calculateCurrentFigures(key: string) {
    ...
    for (let i = 0; i < Object.keys(currentDayData).length; i++) {
      const chainId = Object.keys(currentDayData)[i]
      if (activeChains?.includes(Number(chainId))) {
        total += Number(currentDayData[chainId].data[key])
      }
    }
  ...
  function calculateOneDayFigures(key: string) {
    ...
    for (let i = 0; i < Object.keys(dayOldData).length; i++) {
      const chainId = Object.keys(dayOldData)[i]
      if (activeChains?.includes(Number(chainId))) {
        total += Number(dayOldData[chainId].data[key])
      }
    }
  ...
  function checkDatasInitialized() {
    ...
    for (let i = 0; i < Object.keys(currentDayData).length; i++) {
      const chainId = Object.keys(currentDayData)[i]
      total += currentDayData[chainId].initialized === true ? 1 : 0
    }
    for (let i = 0; i < Object.keys(dayOldData).length; i++) {
      const chainId = Object.keys(dayOldData)[i]
      total += dayOldData[chainId].initialized === true ? 1 : 0
    }
  ...
  function checkChartDataInitialized() {
    ...
    for (let i = 0; i < Object.keys(chartData).length; i++) {
      const chainId = Object.keys(chartData)[i]
      total += chartData[chainId].initialized === true ? 1 : 0
    }
